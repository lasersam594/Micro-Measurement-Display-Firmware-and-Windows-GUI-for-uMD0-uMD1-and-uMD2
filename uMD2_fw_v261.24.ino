...
// V261.24 (1) Added 16 bit Phase option, (2) new firmware version format XXX.YY.  Must use 29-Jan-2025 uMD GUI or later.
...
// around 0.01 mm/second and normal 79 or 158 nm resolution (depending on interferometer optics) above that.
...
#define FirmwareSubVersion 24   // Now numeric in XXX.YY format.
...
#define Phase_16bit_Flag 2      // Set to 1 to tell the GUI to use 16 bit Phase values.
#define FirmwareVersion_Flag 4  // Set to 2 to tell the GUI to use the new Version/Subversion format.
...
int32_t Previous_Phase_Offset = 0;
...
// Numder of axes LED blink code
  ...
  if (LowSpeedCodeSelect == 1) {           // Send firmware version
    LowSpeedCode = 10;
    LowSpeedData = FirmwareVersion + (FirmwareSubVersion << 16);
  }
  ...
  else if (LowSpeedCodeSelect == 10) {     // Firmware_Flags
    LowSpeedCode = 112;
    LowSpeedData = Phase_16bit_Flag;       // Tells the GUI to use 16 bit Phase.
    LowSpeedData |= FirmwareVersion_Flag;  // Tells the GUI to use the fraction for Firmware Subversion
  }
...
#if Phase_16bit_Flag == 2
      Phase_Offset = (Phase_Average_float * 65536) / MEAS_Period_Average_float;  // Average of phase values range ~0 to 65535 (might go slightly below)
      Phase = (Phase_Offset & 0xffff) - 32768;
#else
      Phase_Offset = (Phase_Average_float * 256) / MEAS_Period_Average_float;    // Average of phase values range ~0 to 255 (might go slightly below)
      Phase = (Phase_Offset & 0xff) - 128;
#endif
...
#if Phase_16bit_Flag == 2
       if ((phase < -25000) && (previous_phase > 25000)) { Phase_Drop = true; Test_Flags |= 16; }
  else if ((phase > 25000) && (previous_phase < -25000)) { Phase_Jump = true; Test_Flags |= 32; }
