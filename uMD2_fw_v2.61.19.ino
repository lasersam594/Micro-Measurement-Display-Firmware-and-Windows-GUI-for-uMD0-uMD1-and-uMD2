...
// Version with single axis interpolation capability.  Axes 2 and 3 coded but not tested.  Interpolation
...
#define FirmwareVersion 2.61    // Firmware version used by OLED and GUI About, and hack is used to get it to GUI in "PRC %" field.
#define FirmwareSubVersion .19  // Now numeric.  Hack is to get fractional version number to GUI in "COM %" field.
...
void setup() {
  ...
  Phase_State1 = Normal_State_0;
  Phase_State2 = Normal_State_0;
  Phase_State3 = Normal_State_0;
...
// Axis LED blink code
          if                ((sn & 0x1F0) == 0x040)  digitalWrite (13, 1); // 1 Blink =  1 Axis
     else if ((Axes > 1) && ((sn & 0x1F0) == 0x0C0)) digitalWrite (13, 1); // 2 Blinks = 2 Axes
     else if ((Axes > 2) && ((sn & 0x1F0) == 0x140)) digitalWrite (13, 1); // 3 Blinks = 3 Axes
...
int zero = 0;
    if (Heterodyne > 1) {
      ...
      Serial.printf("%li ", Phase_State1);              // State flags
...
void Interpolation_Execute() {
  ...
  //      digitalWrite (13, 1);
    ...
    Phase3_Offset = (Phase3_Average_x256 + (MEAS3_Period_Average >> 1)) / MEAS3_Period_Average;      // Average of phase values range ~0 to 255 (might go slightly below 0)
...
DISP_LT_DEC = false;
  ...
  Phase_Jump = false;
       ...
       if ((phase < -100) && (previous_phase > 100)) { Phase_Drop = true; Test_Flags |= 16; }
  else if ((phase > 100) && (previous_phase < -100)) { Phase_Jump = true; Test_Flags |= 32; }
