...
// Version with single axis interpolation capability.  Axes 2 and 3 to follow shortly.  Interpolation
// provides nm-scale resolution and precision (accuracy subject to enbironmental compensation) for slew rates of
// around 0.1 mm/second and normal 79 or 158 nm resolution (depending on interferometer optics) above that.
...
#define FirmwareVersion 2.61   // Firmware version used by OLED and GUI About, and hack to get it to GUI in "PRC %" field.
#define FirmwareSubVersion .10 // Now numeric.  Hack to get fractional version number to GUI in "COM %" field.

#define Heterodyne 3           // Set to the number of axes for heterodyne, sample rate always 1 kHz.

#define Multiplier 2           // Integer counts/cycle.  1 for rising edge; 2 for double clocking.  Used for TMR_CTRL_CM value.  Do NOT mess with that!
...
uint OLEDSubVersion = FirmwareSubVersion * 100;
...
int32_t Previous_Displacement_Sent = 0;
int32_t Previous_Displacement1_Sent = 0;
int32_t Previous_Displacement2_Sent = 0;
int32_t Previous_Displacement3_Sent = 0;
...
int32_t Interpolated_Displacement_Sent = 0;
int32_t Interpolated_Displacement1_Sent = 0;
int32_t Interpolated_Displacement2_Sent = 0;
int32_t Interpolated_Displacement3_Sent = 0;

int32_t Displacement_Sent_Difference = 0;
int32_t Displacement1_Sent_Difference = 0;
int32_t Displacement2_Sent_Difference = 0;
int32_t Displacement3_Sent_Difference = 0;

int32_t Interpolated_Displacement_Sent_Difference = 0;
int32_t Interpolated_Displacement1_Sent_Difference = 0;
int32_t Interpolated_Displacement2_Sent_Difference = 0;
int32_t Interpolated_Displacement3_Sent_Difference = 0;
...
int32_t Phase_Difference = 0;
int32_t Phase1_Difference = 0;
int32_t Phase2_Difference = 0;
int32_t Phase3_Difference = 0;

int32_t Interpolated_Displacement = 0;
int32_t Interpolated_Displacement1 = 0;
int32_t Interpolated_Displacement2 = 0;
int32_t Interpolated_Displacement3 = 0;

int32_t Interpolated_Displacement_Difference = 0;
int32_t Interpolated_Displacement1_Difference = 0;
int32_t Interpolated_Displacement2_Difference = 0;
int32_t Interpolated_Displacement3_Difference = 0;
...
uint32_t MEAS1_Edge_Count_Adj = 0;
uint32_t MEAS2_Edge_Count_Adj = 0;
uint32_t MEAS3_Edge_Count_Adj = 0;
...
#define Normal_State_0 0  // DISP NC, Phase SC.
                                // DISP DEC, Phase Drop.
                                // DISP INC, Phase Jump.
...
void setup() {
  ...
  u8x8.drawTile(1, 0, 1, tiles);              // Needed for tail of micro symbol
...
Interpolation_Execute();
  ...
  if (LowSpeedCodeSelect == 1) {           // Send firmware version
    LowSpeedCode = 10;
    LowSpeedData = FirmwareVersion * 100;
  }
  ...
  else if (LowSpeedCodeSelect == 4) {             // Firmware version hack (Was # CPU clocks spent in capture and analysis)
    LowSpeedCode = 121;
    LowSpeedData = FirmwareVersion * 655.3938;
  }

  else if (LowSpeedCodeSelect == 5) {             // Firmware sub-version hack (Was # CPU clocks spent in communications)
    LowSpeedCode = 122;
    LowSpeedData = FirmwareSubVersion * 656.3705;
  }
  ...
  // Low speed codes 101, 102, 111, 112 available for diagnostics
    ...
    //    LowSpeedData = (REF_Period_Average & 0xff) + ((REF_to_MEAS1_Average & 0xff) << 8);  // Old diagnostic thing
  }
  ...
  if (Serial.availableForWrite() > 256) {
    Serial.printf("%li ", REF);                      // 1: REF
    Serial.printf("%li ", MEAS1);                    // 2: MEAS1
    Serial.printf("%li ", Displacement1_Sent);       // 3: displacement1);          // 3: Displacement 1
    Serial.printf("%li ", displacement1);            // 4: Velocity Count 1
    Serial.printf("%li ", Phase1);                   // 5: Phase 1
    Serial.printf("%llu ", sequenceNumber);          // 6: Sequence Number
    Serial.printf("%li ", LowSpeedCode);             // 7: LowSpeedCode
    Serial.printf("%li", LowSpeedData);              // 8: LowSpeedData
...
int zero = 0;
    if (Heterodyne > 1) {
      Serial.printf(" %li " , Exit);                   // State exit identifier   Serial.printf(" State: ");  
      Serial.printf("%li ", Phase_State);              // State flags
      Serial.printf("%li ", zero); // Phase2);              // 5: Phase 2
      Serial.printf("%li ", zero); // velocity2);           // 11: Velocity Count 2
      Serial.printf("%li ", zero); // Displacement1_Sent);               // 13: MEAS3
      Serial.printf("%li ", zero); // displacement1);       // 14: Displacement 3
  ...
  if (Serial.availableForWrite() > 256) {
    ...
    Serial.printf(" Phase: ");
    Serial.printf("%4li ", Phase1);                  // Phase 1
    Serial.printf(" Exit: ");
    Serial.printf("%3li " , Exit);                   // State exit identifier   Serial.printf(" State: ");  
    Serial.printf("Goto: %2li ", Phase_State);             // State flags
    ...
    Serial.printf(" DSD");
    Serial.printf(" %4li ", Displacement1_Sent_Difference);
    Serial.printf(" PD");
    Serial.printf(" %4li ", Phase1_Difference);
    Serial.printf(" IDS");
    Serial.printf(" %4li ", Interpolated_Displacement1_Sent);
    Serial.printf(" IDSD");
    Serial.printf(" %4li ", Interpolated_Displacement1_Sent_Difference);
...
void Interpolation_Execute() {
  ...
  if (No_REF != 0x12345) { // REF OK)
    ...
    if (REF_Edge_Count > MEAS1_Edge_Count) { REF_Edge_Count_Adj = MEAS1_Edge_Count; MEAS1_Edge_Count_Adj = MEAS1_Edge_Count; }
    if (REF_Edge_Count < MEAS1_Edge_Count) { MEAS1_Edge_Count_Adj = REF_Edge_Count; REF_Edge_Count_Adj = REF_Edge_Count; }

    // Calculate average REF period.  Difference between first and last REF edge divided by # REF edges
    if (REF_Edge_Count_Adj >= 4) {
      REF_Period_Total = REF_Edge_Positions[REF_Edge_Count_Adj - 1] - REF_Edge_Positions[0];
      REF_Period_Average_x1024 = ((REF_Period_Total << 10) + 512) / REF_Edge_Count_Adj;               // x1024 to maintain precision without FP
    ...
    if (MEAS1_Edge_Count_Adj >= 4) {
      MEAS1_Period_Total = MEAS1_Edge_Positions[MEAS1_Edge_Count_Adj - 1] - MEAS1_Edge_Positions[0];
      MEAS1_Period_Average_x1024 = ((MEAS1_Period_Total << 10) + 512) / MEAS1_Edge_Count_Adj;         // x1024 to maintain precision without FP
    ...
    Phase1_Total = 0;
    for (j = 1; j < MEAS1_Edge_Count_Adj; j++) {
  ...
  //      digitalWrite (13, 1);
    ...
    Phase_Test(Phase1, Previous_Phase1);
    DISP_Test(displacement1, previous_displacement1);

    Phase_State = Phase_State1;
    Next_State(displacement1);
    Phase_State1 = Phase_State;

    Previous_Displacement1_Sent = Displacement1_Sent;
    Displacement1_Sent = Displacement_Sent;

#if Heterodyne > 1
    // Force # REF edges to equal # MEAS edges
    if (REF_Edge_Count > MEAS2_Edge_Count) { REF_Edge_Count_Adj = MEAS2_Edge_Count; MEAS2_Edge_Count_Adj = MEAS2_Edge_Count; }
    if (REF_Edge_Count < MEAS2_Edge_Count) { MEAS2_Edge_Count_Adj = REF_Edge_Count; REF_Edge_Count_Adj = REF_Edge_Count; }
    ...
    // Calculate average MEAS2 period.  Difference between first and last MEAS2 edge divided by # MEAS2 edges
    if (MEAS2_Edge_Count_Adj >= 4) {
      MEAS2_Period_Total = MEAS2_Edge_Positions[MEAS2_Edge_Count_Adj - 1] - MEAS2_Edge_Positions[0];
      MEAS2_Period_Average_x1024 = ((MEAS2_Period_Total << 10) + 512) / MEAS2_Edge_Count_Adj;         // x1024 to maintain precision without FP
      MEAS2_Period_Average = MEAS2_Period_Average_x1024 >> 10;
    }

    // Initialize sampling variables for averaging

    // THE Phase 2 calculation. :-)
    Previous_Phase2_Offset = Phase2_Offset;
    Previous_Phase2 = Phase2;

    Phase2_Total = 0;
    for (j = 1; j < MEAS2_Edge_Count_Adj; j++) {
      Phase2_Total += (MEAS2_Edge_Positions[j] - REF_Edge_Positions[j]);                          // Sum of phase values
    }

    Phase2_Total_x256 = (Phase2_Total << 8);                                                      // Sum of phase values * 256
    Phase2_Average_x256 = (Phase2_Total_x256 + (MEAS2_Edge_Count_Adj >> 1)) / MEAS2_Edge_Count_Adj;       // Average of phase values * 256
    Phase2_Offset = (Phase2_Average_x256 + (MEAS2_Period_Average >> 1)) / MEAS2_Period_Average;   // Average of phase values range ~0 to 255 (might go slightly below 0)

    // Boundary correction kludges

  //      digitalWrite (13, 1);
 
    Phase2 = (Phase2_Offset & 0xff) - 128;

    Test_Flags = 0;  // DISP and Phase change flags
    Phase_Test(Phase2, Previous_Phase2);
    DISP_Test(displacement2, previous_displacement2);

    Phase_State = Phase_State2;
    Next_State(displacement2);
    Phase_State2 = Phase_State;

    Previous_Displacement2_Sent = Displacement2_Sent;
...
#if Heterodyne > 2
    // Force # REF edges to equal # MEAS edges
    if (REF_Edge_Count > MEAS3_Edge_Count) { REF_Edge_Count_Adj = MEAS3_Edge_Count; MEAS3_Edge_Count_Adj = MEAS3_Edge_Count; }
    if (REF_Edge_Count < MEAS3_Edge_Count) { MEAS3_Edge_Count_Adj = REF_Edge_Count; REF_Edge_Count_Adj = REF_Edge_Count; }
    ...
    // Calculate average MEAS3 period.  Difference between first and last MEAS3 edge divided by # MEAS3 edges
    if (MEAS3_Edge_Count_Adj >= 4) {
      MEAS3_Period_Total = MEAS3_Edge_Positions[MEAS3_Edge_Count_Adj - 1] - MEAS3_Edge_Positions[0];
      MEAS3_Period_Average_x1024 = ((MEAS3_Period_Total << 10) + 512) / MEAS3_Edge_Count_Adj;         // x1024 to maintain precision without FP
      MEAS3_Period_Average = MEAS3_Period_Average_x1024 >> 10;
    }

    // Initialize sampling variables for averaging

    // THE Phase 2 calculation. :-)
    Previous_Phase3_Offset = Phase3_Offset;
    Previous_Phase3 = Phase2;

    Phase3_Total = 0;
    for (j = 1; j < MEAS3_Edge_Count_Adj; j++) {
      Phase3_Total += (MEAS3_Edge_Positions[j] - REF_Edge_Positions[j]);                          // Sum of phase values
    }

    Phase3_Total_x256 = (Phase3_Total << 8);                                                      // Sum of phase values * 256
    Phase3_Average_x256 = (Phase3_Total_x256 + (MEAS3_Edge_Count_Adj >> 1)) / MEAS3_Edge_Count_Adj;       // Average of phase values * 256
    Phase3_Offset = (Phase2_Average_x256 + (MEAS3_Period_Average >> 1)) / MEAS3_Period_Average;   // Average of phase values range ~0 to 255 (might go slightly below 0)

    // Boundary correction kludges

  //      digitalWrite (13, 1);
 
    Phase3 = (Phase3_Offset & 0xff) - 128;

    Test_Flags = 0;  // DISP and Phase change flags
    Phase_Test(Phase3, Previous_Phase3);
    DISP_Test(displacement3, previous_displacement3);

    Phase_State = Phase_State3;
    Next_State(displacement3);
    Phase_State3 = Phase_State;

    Previous_Displacement3_Sent = Displacement3_Sent;
...
/*
    ...
    Displacement1_Sent_Difference = Displacement1_Sent - Previous_Displacement1_Sent; // Diagnostic outputs still buggy
    Phase1_Difference = ((Phase1 + 128) & 0xFF) - ((Previous_Phase1 + 128) & 0xFF);

    Interpolated_Displacement1_Sent = (Displacement1_Sent << 8) - ((Phase1 + 128) & 0xFF);
    Interpolated_Displacement1_Sent_Difference = (Displacement1_Sent_Difference << 8) - ((Phase1_Difference) & 0xFF);
*/
  }
}
#endif
...
bool DISP_DEC = LOW;
bool DISP_INC = LOW;
bool DISP_LT_DEC = LOW;
bool DISP_GT_INC = LOW;

// Test for polarity change in Phase and set flags
void Phase_Test(int phase, int previous_phase) {
  Phase_SC = LOW;
  Phase_Drop = LOW;
  Phase_Jump = LOW;
       if ((phase < -100) && (previous_phase > 100)) { Phase_Drop = HIGH; Test_Flags |= 16; }
  else if ((phase > 100) && (previous_phase < -100)) { Phase_Jump = HIGH; Test_Flags |= 32; }
  else                                               { Phase_SC = HIGH;   Test_Flags |= 8; }
}

// Test for change in Displacement and set flags
void DISP_Test(int32_t displacement, int32_t previous_displacement) {
  DISP_NC = LOW;
  DISP_DEC = LOW;
  DISP_INC = LOW;
  DISP_GT_INC = LOW;
  DISP_LT_DEC = LOW;
       if ((displacement - previous_displacement) == -1) { DISP_DEC = HIGH; Test_Flags |= 2; }
  else if ((displacement - previous_displacement) == 1)  { DISP_INC = HIGH; Test_Flags |= 4; }
  else if ((displacement - previous_displacement) < -1)  { DISP_LT_DEC = HIGH; Test_Flags |= 64; }
  else if ((displacement - previous_displacement) > 1)   { DISP_GT_INC = HIGH; Test_Flags |= 128; }
  else                                                   { DISP_NC = HIGH; Test_Flags |= 1; }
}
...
void Next_State(int displacement) {
  switch (Phase_State) {
  case Normal_State_0: // Normal State 0. 
         if ((DISP_NC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 0;}       //
    else if ((DISP_NC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Bogus_State_2; Displacement_Sent = displacement - 1; Exit = 1; }   //
    else if ((DISP_NC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Bogus_State_1; Displacement_Sent = displacement + 1; Exit = 2; }   //

    else if ((DISP_DEC == HIGH) && (Phase_SC == HIGH))   { Phase_State = Bogus_State_3; Displacement_Sent = displacement + 1; Exit = 3; }   //    
    else if ((DISP_DEC == HIGH) && (Phase_Drop == HIGH)) { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 4; }      //
    else if ((DISP_DEC == HIGH) && (Phase_Jump == HIGH)) { Phase_State = Bogus_State_5; Displacement_Sent = displacement - 1; Exit = 5; }   //

    else if ((DISP_INC == HIGH) && (Phase_SC == HIGH))   { Phase_State = Bogus_State_4; Displacement_Sent = displacement - 1; Exit = 6; }   //
    else if ((DISP_INC == HIGH) && (Phase_Drop == HIGH)) { Phase_State = Bogus_State_6; Displacement_Sent = displacement; Exit = 7; }       //
    else if ((DISP_INC == HIGH) && (Phase_Jump == HIGH)) { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 8; }      //
    break;

  case Bogus_State_1: // DISP_NC and Phase_Jump 
         if ((DISP_NC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Bogus_State_1; Displacement_Sent = displacement + 1; Exit = 10; }  //
    else if ((DISP_NC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 11; }     //
    else if ((DISP_NC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 12; }     //

    else if ((DISP_DEC == HIGH) && (Phase_SC == HIGH))   { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 13; }     //    
    else if ((DISP_DEC == HIGH) && (Phase_Drop == HIGH)) { Phase_State = Bogus_State_3; Displacement_Sent = displacement - 1; Exit = 14; }  //    
    else if ((DISP_DEC == HIGH) && (Phase_Jump == HIGH)) { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 15; }     //

    else if ((DISP_INC == HIGH) && (Phase_SC == HIGH))   { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 16; }     //    
    else if ((DISP_INC == HIGH) && (Phase_Drop == HIGH)) { Phase_State = Bogus_State_4; Displacement_Sent = displacement - 1; Exit = 17; }  //    
    else if ((DISP_INC == HIGH) && (Phase_Jump == HIGH)) { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 18; }     //
    break;

  case Bogus_State_2: // DISP_NC and Phase_Drop
         if ((DISP_NC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Bogus_State_2; Displacement_Sent = displacement - 1; Exit = 20; }  //
    else if ((DISP_NC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 21; }     //
    else if ((DISP_NC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 22; }     //

    else if ((DISP_INC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 23; }    //
    else if ((DISP_INC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 24; }    //
    else if ((DISP_INC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Bogus_State_5; Displacement_Sent = displacement; Exit = 25; }     //

    else if ((DISP_DEC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 26; }    //
    else if ((DISP_DEC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 27; }    //
    else if ((DISP_DEC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Bogus_State_3; Displacement_Sent = displacement + 1; Exit = 28; } //

    else                                                  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 29; }    //
    break;

  case Bogus_State_3: // DISP_DEC and Phase_SC
         if ((DISP_NC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Bogus_State_3; Displacement_Sent = displacement + 1; Exit = 30; }  //
    else if ((DISP_NC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 31; }     //
    else if ((DISP_NC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 32; }     //

    else if ((DISP_INC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 33; }    //
    else if ((DISP_INC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Bogus_State_2; Displacement_Sent = displacement - 1; Exit = 34; } //
    else if ((DISP_INC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Bogus_State_1; Displacement_Sent = displacement + 1; Exit = 35; } //

    else                                                  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 36; }    //
    break;

  case Bogus_State_4: // DISP_INC and Phase_SC
         if ((DISP_NC == HIGH) && (Phase_SC == HIGH))    { Phase_State = Bogus_State_4; Displacement_Sent = displacement - 1; Exit = 40; }  //
    else if ((DISP_NC == HIGH) && (Phase_Drop == HIGH))  { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 41; }     //
    else if ((DISP_NC == HIGH) && (Phase_Jump == HIGH))  { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 42; }     //

    else if ((DISP_DEC == HIGH) && (Phase_SC == HIGH))   { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 43; }     //
    else if ((DISP_DEC == HIGH) && (Phase_Drop == HIGH)) { Phase_State = Error_State_15; Displacement_Sent = displacement; Exit = 44; }     //
    else if ((DISP_DEC == HIGH) && (Phase_Jump == HIGH)) { Phase_State = Bogus_State_1; Displacement_Sent = displacement + 1; Exit = 45; }  //
    break;
