// *************************************************************************************************************
// *                                   Homodyne Firmware for uMD1                                              *
// *************************************************************************************************************

#define FirmwareVersion 5901

#define Homodyne 1           // Set for use with homodyne interferometers, 1 for 1 axis, 2 for 2 axes, 3 for 3 axes, 0 for heterodyne mode.
                             // Sample rate for homodyne always 732.42 Hz.
                             // Homodyne mode uses the counters for axes 1 and 2; polling in the interrupt routine for axis 3.

#define HomodyneMultiplier 16 // Set to homodyne interferometer integer counts/cycle.  4 is Quadpulse and Renishaw Coarse; Fine  is 16.

#define V10_PCB 0            // Set to 1 only for compatibility with SG-uMD1 V1.0 PCB.  (Only affects LED3.)
   
// #define UseSensors 1      // Uncomment this line for sensor support in setup only.
// #define UseSensorsInLoop  // Also uncoment this line for sensor measurements every few minutes.
...
// V56.14 BMP (T,P) and DHT (T,H) sensors acquiried only after reset; SHT sensor (T,H) in loop.
//        LED3 shows duty cycle of data capture, short pause, duty cycle of communications, long pause.
// V56.16 BMP and DHT in loop if UseSensorsInLoop is defined; check every few minutes.  Still locks up after awhile.
// V56.17 Low speed 3rd axis added for homodyne using direct polling of quad signals.
// V58.10 Fixed multi-axis parameters to remedy crashes due to divide by 0.
// V58.30 Changed LED3 function to heartbeat for compatibility with the SG-uMD1 V1.0 PCB.
// V59.01 Adds homodyne multiplier so that firmware tells GUI what resolution to use, added LED3 option for compatibility with chipKit, SG-uMD1 V1.0 and V1.2.
...
//  #include <Wire.h>
  ...
  SHT2xClass sht20;     // instantiate humidity/temperature sensor object
#endif
...
bool shtPresent = true;       // indicator of whether SHT20 sensor is present.
...
#define FIFO_SIZE 100 // size of circular buffer
...
static bool Axis3PinA;
static bool Axis3PinB;
  
static int oldState = 0;
static int newState = 0;
static int Error = 0;
static bool V10 = V10_PCB; // Set to 1 only for compatibility with SG-uMD1 V1.0 PCB

static INT64 HAxis3Displacement = 0; // Position counter for third (slow) homodyne axis
...
{ // microchip timer reference http://ww1.microchip.com/downloads/en/DeviceDoc/61105E.pdf
 ...
 // Averaging values for REF/MEAS and PORTB.  Total must be less than or equal to 32.
    ...
    static INT32 NumberOfMEAS2Averages = 1;  // # REF/MEAS2 averages.  Valid range is 0-32.
    static INT32 NumberOfMEAS3Averages = 1;  // # REF/MEAS3 averages.  Valid range is 0-32.
    static INT32 TotalMEASAverages = 0;
    static INT32 NumberOfPORTBAverages = 20; // # PORTB averages.  Valid range is 0-31.  It may be better to be even.

#ifdef Homodyne
    NumberOfPORTBAverages = 0;
#endif
    ...
    if ((SequenceNumber & 0x600) == 0x200 )
      { 
        digitalWrite(PIN_LED3, !V10); // Turn on LED3 at start of axis capture and analysis
      }
     ...
     for (m = 0;m < NumberOfAxes;m++)
        {
          ...
          for(k = 0;k < AxisParam[m][0];k++) // for number of MEASm averages
            {
...
#ifdef Homodyne
   if (Homodyne > 0) // Don't mess with clock if in homodyne mode
#else
   if (ClkSkipCount > 0)
#endif
     {
...
#ifdef Homodyne
// Acquire Quad data for third (slow) homodyne axis
     ...
     if (Homodyne == 3)
       {
        ...
        if (digitalRead(10) == HIGH) newState = 1;
        if (digitalRead(18) == HIGH) newState |= 2;

        if (oldState == 0)
          {
            if (newState == 0) {}                         // No chnage.
            else if (newState == 1)HAxis3Displacement++;  // Increment position.
            else if (newState == 2)HAxis3Displacement--;  // Decrement position.
            else Error = 1;                               // Error.
          }

        else if (oldState == 1)
          {
            if (newState == 1) {}                         // No change.
            else if (newState == 0)HAxis3Displacement--;  // Decrement position.
            else if (newState == 3)HAxis3Displacement++;  // Increment position.
            else Error = 1;                               // Error.
          }

        else if (oldState == 2)
          {
            if (newState == 2) {}                         // No change.
            else if (newState == 0)HAxis3Displacement++;  // Increment position.
            else if (newState == 3)HAxis3Displacement--;  // Decrement position.
            else Error = 1;                               // Error.
          }

        else if (oldState == 3)
          {
            if (newState == 3) {}                         // No change.
            else if (newState == 2)HAxis3Displacement++;  // Increment position.
            else if (newState == 1)HAxis3Displacement--;  // Deccrement position.
            else Error = 1;                               // Error.
          }
      }
#endif
...
// Fill in values for homodyne mode if needed
#ifdef Homodyne
    MEASFrequencyCount[1] = 0;
    if (Homodyne > 1) MEASFrequencyCount[1] = 1;              // Tells GUI that the second Homodyne axis is active
    TotalDifference[1] = TotalCounter3[1] - TotalCounter3[2]; // Kludge for second Homodyne axis
    MEASFrequencyCount[2] = 0;
    if (Homodyne > 2) MEASFrequencyCount[2] = 1;              // Tells GUI that the third Homodyne axis is active
    TotalDifference[2] = HAxis3Displacement;                  // Third homodyne axis will poll inputs directly
#endif
...
// Fill FIFO
 ...
 //  digitalWrite(PIN_LED1, !digitalRead(PIN_LED1));  // toggle LED heartbeat. This allows visual indication/ use of scope to detect some malfunctions.
 
    digitalWrite(PIN_LED3, V10); // Turn off LED3 at end of axis capture and analysis
...
void setup() {
    OSCConfig (OSC_POSC_PLL,OSC_PLL_MULT_24,OSC_PLL_POST_2,OSC_FRC_POST_2); // Default to 48Mhz
...
// Sensor code - currently only reads after RESET.
    ...
    if ((SHT2x.GetTemperature()) < 0)
      ...
      {
      	temperature1 = SHT2x.GetTemperature()*100;
      }
...
// Setup pins for 3rd homodyne axis
#ifdef Homodyne
 // Inputs for homodyne axis 3
    pinMode(10, INPUT);  // RPA1
    pinMode(18, INPUT);  // RPA4
    pinMode(12, OUTPUT); // Heartbeat LED3
#endif
...
// Timer5 on RPB9 J4-4 (3) (LED3) could also be used for REF but RPB5 is 5 V tolerant.
...
// Determine time spent in this Sensor/Comm routine
  Timer1Start = ReadTimer1();
    if ((SequenceNumber & 0x700) > 0x400)
      {
        digitalWrite(PIN_LED3, !V10); // Turn on LED3 at start of number conversion and USB send
      }
        
    length += sprintf(buffer,"%d ",dataPoint[tail].displayREFFrequencyCount);          // REF Frequency Count
    length += sprintf(buffer+length,"%d ",dataPoint[tail].displayMEAS1FrequencyCount); // MEAS1 Frequency Count
    length += sprintf(buffer+length,"%d ",dataPoint[tail].displayTotalDifference1);    // Distance1
    length += sprintf(buffer+length,"%d ",dataPoint[tail].displayVelocity1Count);      // Velocity1 Count
    length += sprintf(buffer+length,"%d ",dataPoint[tail].displayPhase1);              // Phase1
    length += sprintf(buffer+length,"%d ",dataPoint[tail].displaySequenceNumber);      // Sequence Number
    length += sprintf(buffer+length,"%d ",dataPoint[tail].displayLowSpeedCode);        // Code to specify low speed data type
    length += sprintf(buffer+length,"%d",dataPoint[tail].displayLowSpeedData);         // Low speed data to send back
       
    if (ActiveAxes > 1)
      {
        length += sprintf(buffer+length," %d ",dataPoint[tail].displayMEAS2FrequencyCount); // MEAS2 Frequency Count
        length += sprintf(buffer+length,"%d ",dataPoint[tail].displayTotalDifference2);    // Distance2
        length += sprintf(buffer+length,"%d ",dataPoint[tail].displayVelocity2Count);      // Velocity2 Count
        length += sprintf(buffer+length,"%d ",dataPoint[tail].displayPhase2);              // Phase2
        length += sprintf(buffer+length,"%d ",dataPoint[tail].displayMEAS3FrequencyCount); // MEAS3 Frequency Count
        length += sprintf(buffer+length,"%d ",dataPoint[tail].displayTotalDifference3);    // Distance3
        length += sprintf(buffer+length,"%d ",dataPoint[tail].displayVelocity3Count);      // Velocity3 Count
        length += sprintf(buffer+length,"%d",dataPoint[tail].displayPhase3);               // Phase3
      }
...
//  Serial.flush();
   ...
   // update queue
    if ((FIFO_SIZE - 1) == tail)  // check for wrap around
      ...
      digitalWrite(PIN_LED3, V10); // Check if code gets stuck here

#ifdef UseSensors
#ifdef UseSensorsInLoop
  	sensorDelayCounter++;

   	if (sensorDelayCounter == 100000)
      {     
        if (bmpPresent)
      	  {
//            digitalWrite(PIN_LED1, 1); // Check if code gets stuck here
            temperature1 = bmp.readTemperature()*100; // value to send out over USB
   	        pressure = bmp.readPressure();
//            digitalWrite(PIN_LED1, 0); // Check if code gets stuck here
      	  }
      }
  	if (sensorDelayCounter == 200000)
      {
      	sensorDelayCounter = 0; 
        if (shtPresent)
          {
//            digitalWrite(PIN_LED2, 1); // Check if code gets stuck here
            humidity = SHT2x.GetHumidity()*10;
            temperature1 = SHT2x.GetTemperature()*100;
//            digitalWrite(PIN_LED2, 0); // Check if code gets stuck here
   	      }
      }
#endif
#endif
