...
#define FirmwareSubVersion 33   // Now numeric in XXX.YY format.
...
void Interpolation_Execute() {
  ...
  if (No_REF != 0x12345) { // REF OK)
    ...
    if ((abs(displacement1 - previous_displacement1) <= 1) && (Phase_State1 != Error_State_15)) {
      digitalWrite(Interpolation_OK_LED, On); // Displacement change <= 1 OK indicator
      IOK_LED_Timer = IOK_LED_Timeout;        // Keep it on for a while
      ...
      if (Phase_State1 == Error_State_15) {          // Went into Error State
//        digitalWrite(Interpolation_OK_LED, Off);     // Turn off green Interpolation OK LED
  ...
  //      digitalWrite(Interpolation_Error_LED, Off);  // Tunr off Interpolation Error LED
      }

      Displacement1_Sent = Displacement_Sent;
    }
    else {                                           // Displacement change > 1 excessive indicator
      Displacement1_Sent = displacement1;
      Phase1 = 0;
      Phase1_8bit = 0;
    //  digitalWrite(Interpolation_OK_LED, Off);       // Turn Off interpolation OK LED
    ...
    if ((abs(displacement2 - previous_displacement2) <= 1) && (Phase_State2 != Error_State_15)) {
    ...
    if ((abs(displacement3 - previous_displacement3) <= 1) && (Phase_State3 != Error_State_15)) {
