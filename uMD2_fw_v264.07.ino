...
// V2.64.01 New branch.  Add moving average to V2.61 with interpolation 23-Feb-2025.  Merged Int Displacement and Phase into single Float for averaging.
// V2.64.02 Interpolation and averaging appear incompatible, so need to specify entry and exit conditions.
// V2.64.03 Aborted
// V2.64.04 Cleanup.  Interpolation and averaging produce similar results.  Still not mixable with predictable behavior.
// V2.64.05 All the interpolation loop counts and buffer sizes are now based on the #define Samples_to_Capture value.  Time take a break. ;-)
// V2.64.06 Cleaned up asm block.  Now loads Sample_to_Capture into GPIO_Loop_Count; Overwritten with 12345 error code if no REF edges found (replaces No_REF).
// V2.64.07

#define Heterodyne 1            // Set to the number of axes for heterodyne and interpolation, sample rate always 1 kHz.

// Interpolation control
#define Interpolation 1         // O for none, 1 for enabled.

// PRC% in the GUI provides and indication of how close to the limit the computations are taking.  Modify with caution!
// Interpolation enabled.  Behavior with averaging may be...strange.  There is probably still some inconsistency.
#if Interpolation == 1
#define Captures_to_Average 1       // Number of averages.  The maximum is around 4 with 5,000 samples; 12 with 500 samples.  Try not to go close to or over PRC 97% as bad things msy happen. ;( ;-) 
#define Samples_to_Capture 5000    // Samples per waveform for the asm loop and C code.  The maximum is over 18,000 with no averaging.
#define REF_Sync 1                 // Controls starting point for capture.  May be needed for stability.
#endif
// Interoplation disabled.
#if Interpolation == 0
#define Captures_to_Average 1000    // Number of averages.  The maximum is over 1,000.  The Samples_to_Capture value doesn't matter.
#define Samples_to_Capture 1       // Ignored but memory is allocated if larger.
#define REF_Sync 0                 // Using 0 adds randomness for averaging to avoid stair-step artifacts.
#endif

// OLED selection
#define Old_uMD_GUI 0           // Set to 0 for 16 bit Interpolation with 2025+ GUI, 1 for 8 bit with all GUIs.
#define OLED                    // OLED display of banner and version if defined.
#define OLED_Displacement       // OLED display of SN, Disp1, Disp2, Disp3 if defined.
#define OLED_REF 2              // REF frequency display if defined; Value is text size.  1 (default) and 2 (large) valid.

// Only affects which Interpolation status LED is red and which is green. ;-)
#define PCB_Version 100         // Select PCB version as 100 for V1.00 (really old) or 122 or 123 for V1.22 or V1.23.

// No user or hacker sericeable parts beyond this point. ;-)

#define FirmwareVersion 264     // Firmware version used by OLED and GUI About.
#define FirmwareSubVersion 07   // Now numeric in XXX.YY format.
...
int64_t counter_REF_sample_save = 0;
int64_t counter_MEAS1_sample_save = 0;
int64_t counter_MEAS2_sample_save = 0;
int64_t counter_MEAS3_sample_save = 0;
   
int64_t counter_REF_sample = 0;
int64_t counter_MEAS1_sample = 0;
int64_t counter_MEAS2_sample = 0;
int64_t counter_MEAS3_sample = 0;
...
int32_t REF_Sent =  0;
int32_t MEAS1_Sent =  0;
int32_t MEAS2_Sent =  0;
int32_t MEAS3_Sent =  0;

int32_t Previous_REF_Sent =  0;
int32_t Previous_MEAS1_Sent =  0;
int32_t Previous_MEAS2_Sent =  0;
int32_t Previous_MEAS3_Sent =  0;
...
uint32_t REF_Waveform[Samples_to_Capture + 100];
uint32_t MEAS1_Waveform[Samples_to_Capture + 100];
uint32_t MEAS2_Waveform[Samples_to_Capture + 100];
uint32_t MEAS3_Waveform[Samples_to_Capture + 100];

uint32_t gpioData[Samples_to_Capture + 100];
...
double Displacement1_Float = 0;
double Displacement2_Float = 0;
double Displacement3_Float = 0;

double Displacement1_Average_Float = 0;
double Displacement2_Average_Float = 0;
double Displacement3_Average_Float = 0;
...
uint32_t Waveform_Length = Samples_to_Capture;

double Number_Averaging_Samples = 10.0;
double Fraction_Averaging_Samples = 0.9;
double One_Over_Averaging_Samples = 0.1;

// volatile uint32_t No_REF = Samples_to_Capture; //0x12345;           // Will be changed by any REF activity in the assembly block.  This is probably silly but it doesn't seem to hurt. ;-)
volatile uint32_t GPIO_Sample_Count = Samples_to_Capture;
...
int Phase1_Skip_Timer = 0; // Hold off doing Phase calculation while > 0
int Phase2_Skip_Timer = 0;
int Phase3_Skip_Timer = 0;

bool Phase1_Enable = false; // True if conditions are such that the Phase computation will be valid
bool Phase2_Enable = false;
bool Phase3_Enable = false;

int Displacement1_Averages = 1; // Number of overall average points per sample
int Displacement2_Averages = 1;
int Displacement3_Averages = 1;

double Phase1_Float = 0;
double Phase2_Float = 0;
double Phase3_Float = 0;
...
void setup() {
  ...
  Number_Averaging_Samples = Captures_to_Average;
  Fraction_Averaging_Samples = (Captures_to_Average - 1.0) / Captures_to_Average;
  One_Over_Averaging_Samples = 1.0 / Captures_to_Average;
}

void USBSender() {
  Processing_Start = micros(); // Time stamp for start of processing

    counter_REF_sample_save = counter_REF_sample;
    counter_MEAS1_sample_save = counter_MEAS1_sample;
    counter_MEAS2_sample_save = counter_MEAS2_sample;
    counter_MEAS3_sample_save = counter_MEAS3_sample;
   
    counter_REF_sample = counter_REF;
    counter_MEAS1_sample = counter_MEAS1;
    counter_MEAS2_sample = counter_MEAS2;
    counter_MEAS3_sample = counter_MEAS3;

/***************************** Acquisition and Averaging Block Start ***********************************/

  for (i = 0; i < Number_Averaging_Samples; i++) {

#if REF_Sync == 1 // Normally should be diabled for averaging to avoid stair-step artifacts but may be needed for interpolation.
  // Sync on REF counter input being low in preparation for sync on rising edge since REF starting low is NOT tested for in the assembly block.
    ...
    for (j = 0; j < 100; j++) {  // Loop until REF clock goes low and capture one REF counter value
      if ((IMXRT_GPIO6_DIRECT & 0x400) == 0) {
        j = 100;
      }
    }
    ...
    GPIO_Sample_Count = Samples_to_Capture;

    Waveform_Capture_Assembly_Block ();  // Capture GPIO waveforms if interpolation enabled, one set of counter values if not
  ...
  // Update values
    ...
    velocity1 = displacement1 - previous_displacement1; // Is velocity used anywhere in the GUI??? ;-)
...
#if Interpolation == 1
    Interpolation_Execute(); // This is the Biggie! ;-)
#endif

// GUI:     CurrentValuePhase1 = CDbl(PhaseValue1) / 65536  ' PhaseValue1 from USB
//          currentValue1 = (Convert.ToDouble(AxisData(1, 2)) - CurrentValuePhase1)
//          displacement1 - CurrentValuePhase1

// Combine int Displacement and Phase into one Float for averaging
#if Old_uMD_GUI == 1                              // Use 8 bit Phase1 if it is selected or using old uMD GUI
    Displacement1_Sent *= 256;                    // Displacement1_Sent is int32_t
    Displacement1_Sent -= Phase1_8bit;            // Phase1 is int32_t with range of -128 to +127.  The subtract is because of the idiotic way Phase was defined long long ago.
#else
    Displacement1_Float = Displacement1_Sent;                  // Displacement1_Sent is int32_t
    Phase1_Float = Phase1;
    Displacement1_Float -= (Phase1_Float / 65536.0);           // Phase1 is int32_t with range of -32768 to +32767.  The subtract is because of the idiotic way Phase was defined long long ago (by me).
#endif

    // The actual moving average computation is here
    Displacement1_Average_Float = (Fraction_Averaging_Samples * Displacement1_Average_Float) + (One_Over_Averaging_Samples * Displacement1_Float);
  }

/***************************** Averaging Block Finish ***********************************/

// Frequency counter displays for overall sample since there can be averaging
    REF_Sent = (counter_REF_sample - counter_REF_sample_save) >> Scale_Shift; // Reduction of GUI frequency counter resolution if Multiplier set to 2 (Scale_Shift = 1).;
    MEAS1_Sent = (counter_MEAS1_sample - counter_MEAS1_sample_save) >> Scale_Shift;
    MEAS2_Sent = (counter_MEAS2_sample - counter_MEAS2_sample_save) >> Scale_Shift;
    MEAS3_Sent = (counter_MEAS3_sample - counter_MEAS3_sample_save) >> Scale_Shift;

// Convert Float into  int Displacement1_Sent and Phase1_Sent for USB sender

#if Old_uMD_GUI == 1                                // Use 8 bit Phase1 if it is selected or using old uMD GUI
  Phase1_Sent = -(Displacement1_Sent) % 256;        // Phase1_Sent int32_t
  Displacement1_Sent /=256;                         // Displacement1_Sent int32_t
#else                                               // 16 bit Phase1  
  Phase1_Float = modf(Displacement1_Average_Float, &Displacement1_Float); // Fraction
  Phase1_Float *= 65536;
  Phase1_Sent = -Phase1_Float;
  Displacement1_Sent = Displacement1_Float; // Integer part
#endif

/***************************** Acquisition Block Finish ***********************************/

Processing_Time = micros() - Processing_Start;      // Time spent in acquiring data

/***************************** Housekeeping and Low Speed Data Block Start ***********************************/ // Total time spent here is negligible
...
// Status LED keep-on timers.  Status LEDs are unused Homodyne Axis 3 Quad A/B signals.  RS422 receiver should NOT be present.

  if (IER_LED_Timer > 1) IER_LED_Timer --;  // Keep red Interpolation Error LED on for a while
...
/***************************** Housekeeping and Low Speed Data Block Finish ***********************************/

/***************************** Communications Block Start ***********************************/

Communications_Start = micros(); // Tiome stamp for start of communications

#if Diagnostics == 0  // Normal uMD GUI
  if (Serial.availableForWrite() > 256) {
    Serial.printf("%li ", REF_Sent);                      // 1: REF
    Serial.printf("%li ", MEAS1_Sent);                    // 2: MEAS1
    ...
    if (Heterodyne > 1) {
      Serial.printf(" %li ", MEAS2_Sent);                 // 9: MEAS2
      ...
      Serial.printf("%li ", MEAS3_Sent);                  // 13: MEAS3
...
int zero = 0;
    if (Heterodyne > 0) {
      ...
      Serial.printf("%li ", Phase1_State );           // State flags            // Velocity2
      Serial.printf("%li ", Displacement2_Sent);                   // velocity2);     // 11: Velocity Count 2
      Serial.printf("%li ", Phase2_Sent);            // Phase2);        // 5: Phase 2
      Serial.printf("%li ", Displacement3_Sent);                    // displacement3); // 14: Displacement 3
  ...
  if (Serial.availableForWrite() > 256) {
    ...
    Serial.printf("Goto: %2li ", Phase1_State );     // State flags
...
Communications_Time = micros() - Communications_Start; // Time spent in communications

/***************************** Communications Block Finish ***********************************/
}

void loop() { // Only for OLED and Status LEDs
  OLED_DISPLAY();
  delay(100);
}
...
void Interpolation_Execute() {
  ...
  if ((GPIO_Sample_Count != 12345) && (REF >= 100)) { // REF OK and at least 100 counts/ms)
...
#define Phase_Skip_Timeout 15 // Number of consecutive samples to skip if Phase compuation is invalid

    // Axis 1 Phase calculation
    if ((abs(displacement1 - previous_displacement1) <= 1) && (Phase1_State  != Error_State_15)) {             // Perform conditional Phase1 and Phase1 State computation if abs(displacement1 change) 0 or 1

      Previous_Phase1 = Phase1;
      Phase_Execute (REF_Edge_Count, MEAS1_Edge_Count, MEAS1_Edge_Positions);                                  // Determine new Phase1 based on computed REF and MEAS1 positions
      Phase1 = Phase; Phase1_8bit = Phase_8bit;
   
      Phase_State = Phase1_State;
      Next_State(displacement1, previous_displacement1, Phase1, Previous_Phase1);                              // Determine Phase1 State based on current and previous displacement and Phase1
      Phase1_State = Phase_State;
 
      Displacement1_Sent = displacement1;                                                                      // Default to unadjusted displacement1 should End-Point Regime (EPR) not be valid below

      if (Phase1_Enable == false) {                                                                            // Are we in EPR?
        if ((displacement1 - previous_displacement1) == 0) {
          Phase1_Skip_Timer --;
          if ((Phase1_Skip_Timer <= 0) && (abs(Phase1 < 16384))) {
            Phase1_Enable = true;                                                                              // Enter EPR if Phase_Skip_Timeout consecutive samples with 0 displacement change
            Phase1_State = Normal_State_0;                                                                     // Reset to Normal_State_0
          }
        }
        else Phase1_Skip_Timer = Phase_Skip_Timeout;
      }
      else if (((abs(displacement1 - previous_displacement1)) > 1) || (Phase1_State  == Error_State_15)) {     // If abs(displacement1 change) > 1 or Error_State_15
        Phase1_Enable = false;                                                                                 // Exit EPR
        Phase1_Skip_Timer = Phase_Skip_Timeout;                                                                // Set Phase1_Skip_Timer to Phase_Skip_Timeout
        digitalWrite(Interpolation_Error_LED, On);                                                             // Turn on red Interpolation Error LED
        IER_LED_Timer = IER_LED_Timeout;                                                                       //  and keep it on for a while 
        Phase1_State = Normal_State_0; Phase1 = 0; Phase1_8bit = 0;  Previous_Phase1 = 0;                      // Reset to Normal_State_0, clear Phase1 values
      }
      else {                                                                                                   // This is the ONLY valid outcome! ;-)
        digitalWrite(Interpolation_OK_LED, On);                                                                // Turn on green Interpolation OK LED
        IOK_LED_Timer = IOK_LED_Timeout;                                                                       //  and keep it on for a while
        Displacement1_Sent = Displacement_Sent;                                                                // Use Next_State-corrected Displacement1_Sent.
      }
    }
    else {                                                                                                     // Displacement1 change is > 1, excessive
      Phase1_Enable = false;                                                                                   // Exit EPR
      Phase1_Skip_Timer = Phase_Skip_Timeout;                                                                  // Set Phase1_Skip_Timer to Phase_Skip_Timeout
      Displacement1_Sent = displacement1;                                                                      // Use original displacement1
      Phase1_State = Normal_State_0;  Phase1 = 0; Phase1_8bit = 0; Previous_Phase1 = 0;                        // Reset to Normal_State_0, clear Phase1 values
      digitalWrite(Interpolation_Error_LED, On);                                                               // Turn on interpolation Error LED
      IER_LED_Timer = IER_LED_Timeout;                                                                         //  and keep it on for a while
    }

  #if Heterodyne > 1
  // Axis 2 Phase calculation
    if ((abs(displacement2 - previous_displacement2) <= 1) && (Phase2_State  != Error_State_15)) {             // Perform conditional Phase2 and Phase2 State computation if abs(displacement2 change) 0 or 1

      Previous_Phase2 = Phase2;
      Phase_Execute (REF_Edge_Count, MEAS2_Edge_Count, MEAS2_Edge_Positions);                                  // Determine new Phase2 based on computed REF and MEAS1 positions
      Phase2 = Phase; Phase2_8bit = Phase_8bit;
   
      Phase_State = Phase2_State;
      Next_State(displacement2, previous_displacement2, Phase2, Previous_Phase2);                              // Determine Phase2 State based on current and previous displacement2 and Phase2
      Phase2_State = Phase_State;
 
      Displacement2_Sent = displacement2;                                                                      // Default to unadjusted displacement2 should End-Point Regime (EPR) not be valid below

     if (Phase2_Enable == false) {                                                                             // Are we in EPR?
        if ((displacement2 - previous_displacement2) == 0) {
          Phase2_Skip_Timer --;
          if ((Phase2_Skip_Timer <= 0) && (abs(Phase2 < 16384))) {
            Phase2_Enable = true;                                                                              // Enter EPR if Phase_Skip_Timeout consecutive samples with 0 displacement change
            Phase2_State = Normal_State_0;                                                                     // Reset to Normal_State_0
          }
        }
        else Phase2_Skip_Timer = Phase_Skip_Timeout;
      }

      else if (((abs(displacement2 - previous_displacement2)) > 1) || (Phase2_State  == Error_State_15)) {     // If abs(displacement2 change) > 1 or Error_State_15
        Phase2_Enable = false;                                                                                 // Exit EPR
        Phase2_Skip_Timer == Phase_Skip_Timeout;                                                               // Set Phase2_Skip_Timer to Phase_Skip_Timeout
        digitalWrite(Interpolation_Error_LED, On);                                                             // Turn on red Interpolation Error LED
        IER_LED_Timer = IER_LED_Timeout;                                                                       //  and keep it on for a while 
        Phase2_State = Normal_State_0; Phase2 = 0; Phase2_8bit = 0;  Previous_Phase2 = 0;                      // Reset to Normal_State_0, clear Phase2 values
      }
      else {                                                                                                   // This is the ONLY valid outcome! ;-)
        digitalWrite(Interpolation_OK_LED, On);                                                                // Turn on green Interpolation OK LED
        IOK_LED_Timer = IOK_LED_Timeout;                                                                       //  and keep it on for a while
        Displacement2_Sent = Displacement_Sent;                                                                // Use Next_State-corrected Displacement2_Sent.
      }
    }
    else {                                                                                                     // Displacement2 change is > 1, excessive
      Phase2_Enable = false;                                                                                   // Exit EPR
      Phase2_Skip_Timer = Phase_Skip_Timeout;                                                                  // Set Phase2_Skip_Timer to Phase_Skip_Timeout
      Displacement2_Sent = displacement2;                                                                      // Use original displacement2
      Phase2_State  = Normal_State_0; Phase2 = 0; Phase2_8bit = 0; Previous_Phase2 = 0;                        // Reset to Normal_State_0, clear Phase2 values
  ...
  #if Heterodyne > 2
      // Axis 3 Phase calculation
    if ((abs(displacement3 - previous_displacement3) <= 1) && (Phase3_State  != Error_State_15)) {             // Perform conditional Phase3 and Phase3 State computation if abs(displacement3 change) 0 or 1

      Previous_Phase3 = Phase3;
      Phase_Execute (REF_Edge_Count, MEAS3_Edge_Count, MEAS3_Edge_Positions);                                  // Determine new Phase3 based on computed REF and MEAS3 positions
      Phase3 = Phase; Phase3_8bit = Phase_8bit;
   
      Phase_State = Phase3_State;
      Next_State(displacement3, previous_displacement3, Phase3, Previous_Phase3);                              // Determine Phase3 State based on current and previous displacement and Phase3
      Phase3_State = Phase_State;
 
      Displacement3_Sent = displacement3;                                                                      // Default to unadjusted displacement3 should End-Point Regime (EPR) not be valid below

      if (Phase3_Enable == false) {                                                                            // Are we in EPR?
        if ((displacement1 - previous_displacement3) == 0) {
          Phase3_Skip_Timer --;
          if ((Phase3_Skip_Timer <= 0) && (abs(Phase3 < 16384))) {
            Phase3_Enable = true;                                                                              // Enter EPR if Phase_Skip_Timeout consecutive samples with 0 displacement change
            Phase3_State = Normal_State_0;                                                                     // Reset to Normal_State_0
          }
        }
        else Phase3_Skip_Timer = Phase_Skip_Timeout;
      }
      else if (((abs(displacement3 - previous_displacement3)) > 1) || (Phase3_State  == Error_State_15)) {     // If abs(displacement3 change) > 1 or Error_State_15
        Phase3_Enable = false;                                                                                 // Exit EPR
        Phase3_Skip_Timer == Phase_Skip_Timeout;                                                               // Set Phase3_Skip_Timer to Phase_Skip_Timeout
        digitalWrite(Interpolation_Error_LED, On);                                                             // Turn on red Interpolation Error LED
        IER_LED_Timer = IER_LED_Timeout;                                                                       //  and keep it on for a while 
        Phase3_State = Normal_State_0; Phase3 = 0; Phase3_8bit = 0;  Previous_Phase3 = 0;                      // Reset to Normal_State_0, clear Phase3 values
      }
      else {                                                                                                   // This is the ONLY valid outcome! ;-)
        digitalWrite(Interpolation_OK_LED, On);                                                                // Turn on green Interpolation OK LED
        IOK_LED_Timer = IOK_LED_Timeout;                                                                       //  and keep it on for a while
        Displacement3_Sent = Displacement_Sent;                                                                // Use Next_State-corrected Displacement3_Sent.
      }
    }
    else {                                                                                                     // Displacement3 change is > 1, excessive
      Phase3_Enable = false;                                                                                   // Exit EPR
      Phase3_Skip_Timer = Phase_Skip_Timeout;                                                                  // Set Phase3_Skip_Timer to Phase_Skip_Timeout
      Displacement3_Sent = displacement3;                                                                      // Use original displacement3
      Phase3_State  = Normal_State_0; Phase3 = 0; Phase3_8bit = 0; Previous_Phase3 = 0;                        // Reset to Normal_State_0, clear Phase3 values
...
// Set flags based on displacement and phase changes
...
DISP_LT_DEC = false;
  ...
  else if ((displacement - previous_displacement) < -1)  { DISP_LT_DEC = true; Test_Flags |= 64; return; }
  else if ((displacement - previous_displacement) > 1)   { DISP_GT_INC = true; Test_Flags |= 128; return; }
...
#define Phase_16bit_Threshold 16384
  ...
  if ((DISP_LT_DEC == true) || (DISP_GT_INC == true))    { Phase_State = Normal_State_0; Displacement_Sent = displacement; Exit = 37; return; }  // Reset if abs(displacement change) is > 1
...
void Waveform_Capture_Assembly_Block () {
  ...
  asm volatile("ldr    r0, =0x401dc00a \n\t" // Load address of MEAS2 (TMR1_CNTR0) into r0
               ...
               "ldr    r12, %0         \n\t" // Load GPIO_Loop_Count into r12
               "lsls   r12, r12, #2    \n\t" // Shift left by 2 => multiply by 4
               "subs   r12, r12, #4    \n\t" // Subtract 4 to reduce count by 1 since the loop does count+1
               "add    r10, r10, r12   \n\t" // Add it to r10 used for GPIO loop limit
               "mov    r12, #0x400     \n\t" // Load bit to test in r12
               "mov    r11, #100       \n\t" // Load loop count in r11
    ...
    "NoREFEdgeFound1:                  \n\t" // No high REF clock detected
               "movw   r12, #12345     \n\t" // Load error code - 12345
               "str    r12, %0         \n\t" // No_REF for GPIO_Sample_Count
               "b      Abort           \n\t" // Skip gpioData capture if no REFs
                       
    "REFEdgeFound1:                    \n\t" // Breakout
               "ldrh   r12, [r1], #0   \n\t" // Hold TMR2 by reading TMR2_CNTR0 (REF)
               "ldrh   r12, [r3], #0   \n\t" // Hold TMR4 by reading TMR4_CNTR0 (MEAS1)
               "ldrh   r12, [r0], #0   \n\t" // Hold TMR1 by reading TMR1_CNTR0 (MEAS2)
               "ldrh   r12, [r2], #0   \n\t" // Hold TMR3 by reading TMR3_CNTR0 (MEAS3)

#if Interpolation == 0
               "b      Abort           \n\t" // Skip gpioData capture if no interpolation
#endif
  ...
  // Read GPIO directly for interpolation calculation
    ...
    "Abort:                            \n\t" // Interpolation disabled, no REF edges found, or normal exit from GPIO capture loop 
               "nop                    \n\t" // Placeholder to exit

               : "=m" (GPIO_Sample_Count)                                                                  // Output operand list
               : "r" (gpioData)              // Input operand list
               : "r0", "r1", "r2", "r3", /* "r4", "r5",*/ "r6", "r8", "r9", "r10", "r11", "r12" // Clobber list
              );
}
