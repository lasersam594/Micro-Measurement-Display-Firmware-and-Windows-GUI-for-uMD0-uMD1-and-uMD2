...
void setup() {
  ...
  Serial.println(" Source: Squarewave REF with a period of 16; Destination prefilled with 0xDEADFEEDs");

  // Initialize buffers with FAKE DATA (squarewave) and constant 0xDEADFEEDs
  for (int i = 0; i < BUFFER_SIZE; i++) {
    destBuffer[i] = 0xDEADFEED;
    if ((i & 0X10) != 0) srcBuffer[i] = REF_MASK;
    else srcBuffer[i] = 0;
  }
  ...
  Serial.println(" Source: GPIO_PSR; Destination prefilled with 0xDEADFEEDs");
