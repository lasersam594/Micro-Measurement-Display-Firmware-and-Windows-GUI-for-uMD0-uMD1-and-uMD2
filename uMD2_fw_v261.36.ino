...
// V261.34  Broke ;-(
// V261.35  Broke ;-(
// V261.36  Working on re-entering low slew rate regime for Axis 1.  Seems somewhat better behaved.
...
#define PCB_Version V1.00       // Select PCB version as 1.00 (really old) or V1.22 and V1.23.  Only affects Interpolation status LEDs.
...
#define FirmwareSubVersion 36   // Now numeric in XXX.YY format.
...
#ifdef PCB_Version == V1.00
#define Interpolation_OK_LED 5    // Phase State NOT Error green
#define Interpolation_Error_LED 7 // Displacement change > 1 red
#else
#define Interpolation_OK_LED 7    // Phase State NOT Error green
#define Interpolation_Error_LED 5 // Displacement change > 1 red
#endif
...
void Interpolation_Execute() {
  ...
  if ((No_REF != 0x12345) && (REF >= 100)) { // REF OK and at least 100 counts/ms)
...
#define Phase_Skip_Timeout 10 // Number of consecutive samples to skip if Phase compuation is invalid

    // Axis 1 Phase calculation
    if ((abs(displacement1 - previous_displacement1) <= 1) && (Phase1_State  != Error_State_15)) {             // Perform conditional Phase1 and Phase1 State computation if abs(displacement change) 0 or 1
      ...
      if (Phase1_Enable == false) {                                                                            // Are we in EPR?
        if (((displacement1 - previous_displacement1) == 0) && (Phase1_Skip_Timer > 0)) Phase1_Skip_Timer --;  // No.  Decrement Phase_Skip_Timer if 0 displacement change
        if (Phase1_Skip_Timer <= 0) Phase1_Enable = true;                                                      // Enter EPR if Phase_Skip_Timeout consecutive samples with 0 displacement change
        Phase1_State = Normal_State_0; Phase1 = 0; Phase1_8bit = 0; Previous_Phase1 = 0;                       // Reset to Normal_State_0, clear Phase1 values
      }
      else if (((abs(displacement1 - previous_displacement1)) > 1) || (Phase1_State  == Error_State_15)) {     // If abs(displacement change) > 1 or Error_State_15
        ...
        Displacement1_Sent = Displacement_Sent;                                                                // Use Next_State-corrected Displacement_Sent.
      }
    }
    ...
    if ((abs(displacement2 - previous_displacement2) <= 1) && (Phase2_State  != Error_State_15)) {
    ...
    if ((abs(displacement3 - previous_displacement3) <= 1) && (Phase3_State  != Error_State_15)) {
...
//             "add    r10, #3996      \n\t" // Original 1000 samples.  For the end-of-loop condition. we want Waveform_Length 4 byte values, so the end is at 4000 - 4 = 3996 bytes after the beginning of the array
               "add    r10, #0x5000    \n\t" // 5121 samples.  0x5000: 20480 / 4 = 5120.   Any shift of 8 bit value is valid for the immediate add.
